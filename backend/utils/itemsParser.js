...
function parsePartsFromSheetText(partsText) {
  ...
  blocks.forEach(block => {
    ...
    items.push({
      partId: 'part-' + (partNumber !== 'N/A' ? partNumber : randomUUID().slice(0, 8)),
      partNumber,
      partName,
      name: partName,
...
function parseItemsIssued(rawItems) {
  if (!rawItems) return [];
  ...
  if (Array.isArray(rawItems)) {
    ...
    rawItems.forEach(item => {
      if (typeof item === 'string') {
        ...
        result.push({
          partId: item.partId || item.id || 'part-' + (item.partNumber || randomUUID().slice(0, 8)),
